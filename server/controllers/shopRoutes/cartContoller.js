  ...
  try {
    ...
    if (!productId || !quantity) {
      return res.status(400).json({
        success: false,
        message: "Request must contain all parameters(productId, quantity)",
      });
    }
    ...
    await userCart.save();
    res.status(200).json({ success: true, message: "Item added to your cart!", data: userCart.items, cartId: userCart._id });
  } catch (error) {
...
const getCartItems = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, data: userCart.items, cartId: userCart._id });
  } catch (error) {
    res.status(500).json({ success: false, message: "ServerError:", error });
  }
};
